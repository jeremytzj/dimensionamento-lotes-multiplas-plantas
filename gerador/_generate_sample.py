...
production_slack_period = 5
__DEFAULT_PERIODS = 12  # Horizonte default
...
def gen_routes(
    ...
    routes["unit_freight_cost"] = (
        routes['freight_cost']/250000
    )
    ...
    for _, customer in customers.iterrows():
        ...
        for period in periods:
            ...
            for product in products_with_demand:
                if periods.index(period) >= production_slack_period:
...
with pd.ExcelWriter(__OUTPUT_FILE, engine="openpyxl") as writer:
    pd.DataFrame(data=periods).to_excel(writer, sheet_name="Periods", index=False)
...
print(f"Exported to {__OUTPUT_FILE}")
